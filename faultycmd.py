...
def config():
    ...
    table_config.add_row("Pulse time", f"{faulty_worker.board_configurator.BOARD_CONFIG['pulse_time']}")
    table_config.add_row("Pulse power", f"{faulty_worker.board_configurator.BOARD_CONFIG['pulse_power']}")
...
def faulty(
    ...
    if not faulty_worker.validate_serial_connection():
        typer.secho(f"FaultyCMD could not stablish connection withe the board on: {comport}.", fg=typer.colors.RED)
        return
